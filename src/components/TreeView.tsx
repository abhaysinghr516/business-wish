...
export const defaultTreeData: TreeNode[] = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          { id: "utils.ts", name: "utils.ts", type: "file", icon: <File className="w-4 h-4 text-neutral-400" /> },
          { id: "api.ts", name: "api.ts", type: "file", icon: <Database className="w-4 h-4 text-neutral-400" /> },
          { id: "auth.ts", name: "auth.ts", type: "file", icon: <Shield className="w-4 h-4 text-neutral-400" /> }
        ]
      },
      {
        id: "assets",
        ...
        children: [
          { id: "logo.svg", name: "logo.svg", type: "file", icon: <ImageIcon className="w-4 h-4 text-neutral-400" /> },
          { id: "hero.webp", name: "hero.webp", type: "file", icon: <FileImage className="w-4 h-4 text-neutral-400" /> }
        ]
      },
      { id: "index.tsx", name: "index.tsx", type: "file", icon: <File className="w-4 h-4 text-neutral-400" /> },
      { id: "globals.css", name: "globals.css", type: "file", icon: <File className="w-4 h-4 text-sky-400" /> }
    ]
  },
...
const BasicTreeNode: React.FC<{ node: TreeNode; level: number }> = ({ node, level }) => {
...
export const BasicTreeView: React.FC<BasicTreeProps> = ({ data = defaultTreeData }) => {
  return (
...
const FinderTreeNode: React.FC<{ node: TreeNode; level: number; activeId: string | null; onSelect: (id: string) => void }> = ({ node, level, activeId, onSelect }) => {
...
export const FinderTreeView: React.FC<BasicTreeProps> = ({ data = defaultTreeData }) => {
  const [activeId, setActiveId] = useState<string | null>("src");
...
const LineTreeNode: React.FC<{ node: TreeNode; level: number; isLast: boolean }> = ({ node, level, isLast }) => {
    ...
    <div className="w-full relative">
      ...
      >
        {isFolder ? (
          ...
          <div className="w-5 h-5 mr-2 flex-shrink-0 z-10" /> // Spacer for alignment
        )}
...
export const LineTreeView: React.FC<BasicTreeProps> = ({ data = defaultTreeData }) => {
  return (
...
const SettingsTreeNode: React.FC<{ node: TreeNode; level: number }> = ({ node, level }) => {
  const [isOpen, setIsOpen] = useState(level === 0);
  const isFolder = node.type === "folder";
...
export const SettingsTreeView: React.FC<BasicTreeProps> = ({ data = defaultTreeData }) => {
  return (
